	...
	public void executeGame() {
		...
		while (!gameEnded) {
			for (int p = 0; p < playersNumber; p++) {// Controlla se la plancia deve essere riempita
				...
				try {
					System.out.print(players.get(p).getNickname()
					...
					while (!plank.isChoiceValid(row1, col1)) {
						System.out.print(
								"The given coordinates are invalid. Please write the row, followed by a space, and then the column of the tile you want to pick: ");
					...
					if (choice == 'y') {
						numOfTiles = 2;
						System.out.print(
								"Write the row and the column of the second tile you want to pick, separated by a space, and press enter: ");
						line = scanner.nextLine();
						coords = line.trim().split(" ");
						...
						if (choice == 'y') {
							numOfTiles = 3;
							System.out.print(
									"Write the row and the column of the third tile you want to pick, separated by a space, and press enter: ");
							line = scanner.nextLine();
							coords = line.trim().split(" ");
					...
					while (column < 0 || column > 4 || full == true) {
						...
						if (column < 1 || column > 4) {
							...
							System.out.println(
									"The column you have chosen is already full or cannot contain that many tiles");
							full = true;
						}
					}
					...
					System.out.println(
							"You chose too high numbers for the row and/or the column or you forgot to use the space");
					p = (p - 1) % playersNumber;
					continue;
				}
		...
		for (Player player : players) {
			int punteggio = player.library.score()
